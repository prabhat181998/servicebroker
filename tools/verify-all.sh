...
echo Verify OpenAPI
docker run --rm -v "${REPODIR}":/local openapitools/openapi-generator-cli:latest validate -i /local/openapi.yaml || rc=1

echo Verify Swagger
docker run --rm -v "${REPODIR}":/local openapitools/openapi-generator-cli:latest validate -i /local/swagger.yaml || rc=1
